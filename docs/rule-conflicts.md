...
Origin registry; resolution semantic-review. Fires when: A controlled-technical profile is active and recurring content tempts wording variation.
...
Origin registry; resolution deterministic. Fires when: A controlled-technical profile is active over descriptive prose and sentence-length variation is preferred.
...
Origin registry; resolution deterministic. Fires when: An em dash would be replaced while a controlled-technical profile bans the semicolon.
...
Origin registry; resolution semantic-review. Fires when: An effort estimate is requested or expected and no defensible range exists.
...
Origin composition; resolution semantic-review. Fires when: The request is an informational question or explanation rather than an actionable task.
...
Origin composition; resolution semantic-review. Fires when: A claim carries justified uncertainty while the anti-hedging rule is active.
...
- Tie-break: Operational test: state is a past-tense outcome bound to a reader-checkable artifact or gate; diary is exploration narration that produces no reader-checkable state change. A mixed sentence keeps the outcome clause and drops the exploration clause.
    ...
    Expected rewrite: `The fix version is unconfirmed because the vendor changelog is unavailable.`
...
Origin composition; resolution semantic-review. Fires when: The request asks for a conceptual or definitional explanation with no natural number, name, or date.
...
Origin composition; resolution semantic-review. Fires when: The user explicitly requests a casual, creative, or organizational voice.
...
Origin composition; resolution semantic-review. Fires when: The user requests a hypothetical example or scenario.

- Winner: band `contract` (`contract`).
- Losing behavior: Refusing a requested hypothetical, or presenting an invented scenario as a real event.
- Permitted substitute: Provide the scenario labeled as hypothetical; never present invented events as history.
...
Origin composition; resolution deterministic. Fires when: An explicit user or organizational style directive sets a different regional spelling.

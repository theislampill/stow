...
| `dictionary_lookup.py` | G2 lookup helper | Explicit controlled-profile caller | Reports closed dictionary membership, preserved alternatives, and explicitly listed forms; sparsely scans caller-labelled editable segments | Extraction accounting, schema, collision, protected-segment, lookup, package, and install tests | Cold standard-library module plus compressed data read only when called; zero ordinary-turn prompt cost | Does not decide sense, part of speech, technical-term authority, replacement meaning, conformance, or final-output custody |

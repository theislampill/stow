...
| D-19 | Stale counts, versions, hashes, claims | Callable-count phrases pinned to the runtime; catalog statuses pinned to the registry; version pinned to the manifest; artifact freshness machine-checked | mechanically checked | `tests/test_doc_lint.py`, `tests/test_readme_catalog.py`, `tests/test_build.py`, `tests/test_repo_hygiene.py` |
